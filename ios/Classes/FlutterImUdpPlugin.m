...
@interface FlutterImUdpPlugin()<MessageQoSEvent,ChatMessageEvent,ChatBaseEvent>
@property (nonatomic) BOOL _init;
@property (nonatomic,strong) FlutterMethodChannel *channel;
@end

@implementation FlutterImUdpPlugin
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
  FlutterMethodChannel* channel = [FlutterMethodChannel
      methodChannelWithName:@"flutter_im_udp"
            binaryMessenger:[registrar messenger]];
  FlutterImUdpPlugin* instance = [[FlutterImUdpPlugin alloc] init];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
   ...
   }else if([call.method isEqualToString:@"initMobileIMSDK"]) {
...
- (NSDictionary *)getLoginInfo {
    ...
    [self addObjectForDic:argument key:@"currentLoginUserId" value:[ClientCoreSDK sharedInstance].currentLoginInfo.loginUserId];
    [self addObjectForDic:argument key:@"currentLoginToken" value:[ClientCoreSDK sharedInstance].currentLoginInfo.loginToken];
    [self addObjectForDic:argument key:@"currentLoginExtra" value:[ClientCoreSDK sharedInstance].currentLoginInfo.extra];
    NSNumber *firstLoginTime = [NSNumber numberWithLong: [ClientCoreSDK sharedInstance].currentLoginInfo.firstLoginTime];
    [self addObjectForDic:argument key:@"firstLoginTime" value:firstLoginTime];
    return argument;
}
...
- (void)getCurrentLoginInfo:(FlutterMethodCall*)call result:(FlutterResult)result {
    // 获取当前登录信息
    if([ClientCoreSDK sharedInstance].currentLoginInfo != nil) {
...
{
    NSLog(@"【DEBUG_UI】已收到服务端的\"被踢\"指令，code：%d", kickoutInfo.code);

    NSString *reason = kickoutInfo.reason ? kickoutInfo.reason : @"KICKOUT_FOR_DUPLICATE_LOGIN";

    [self.channel invokeMethod:@"kickout" arguments:@{@"errorCode":[NSNumber numberWithInt:kickoutInfo.code],@"errorMsg": reason}];
}
